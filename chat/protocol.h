...
typedef struct STRU_GET_USERICON_RS {
    ...
    STRU_GET_USERICON_RS()
        : type(_DEF_PROTOCOL_GETUSERICON_RS)
        , userid(0)
        , flag(USERINFO) {
...
struct STRU_FILE_INFO_RQ {
    typedef int PackType;
    STRU_FILE_INFO_RQ()
        : type(_DEF_PROTOCOL_FILE_INFO_RQ)
        , fileSize(0)
        , senderId(0)
        , receiverId(0) {
        memset(fileId, 0, _MAX_FILE_PATH_SIZE);
 ...
 * @brief 文件信息回复
 */
union STRU_FILE_INFO_RS {
    struct STRU_FILE_INFO_FALSE_RS { // 服务器伪应答
        enum Result {
            NEEDUPLOAD,
            NOTNEEDUPLOAD
        };
        STRU_FILE_INFO_FALSE_RS() 
            : type(FILE_INFO_RS)
            , result(NEEDUPLOAD) {
                memset(filePath, 0, _MAX_FILE_PATH_SIZE);
            }
    ...
    struct STRU_FILE_INFO_TRUE_RS { // 客户端B应答
        enum Result {
            ACCEPT,
            REFUSE
        };
        STRU_FILE_INFO_TRUE_RS()
            : type(FILE_INFO_RS)
            , result(ACCEPT)
            , senderId(0)
            , receiverId(0) {
...
struct STRU_FILE_BLOCK_RQ {
    typedef int PackType;
    STRU_FILE_BLOCK_RQ()
        : type(_DEF_PROTOCOL_FILE_BLOCK_RQ)
        , blockSize(0)
        , senderId(0)
        , receiverId(0) {
        memset(fileId, 0, _MAX_FILE_PATH_SIZE);
...
struct STRU_FILE_BLOCK_RS {
    typedef int PackType;
    STRU_FILE_BLOCK_RS()
        : type(_DEF_PROTOCOL_FILE_BLOCK_RS)
        , nResult(_file_block_recv_success)
        , uuid(0)
        , friendid(0) {
        memset(fileId, 0, _MAX_FILE_PATH_SIZE);
    }
